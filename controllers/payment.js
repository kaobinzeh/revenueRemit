...
exports.getPayments = asyncHandler(async (req, res, next) => {
  ...
  else if (req.baseUrl.split("/")[3] === "tariff") {
    const payment = await Payment.find({
  ...
  else if (req.originalUrl.split("/")[4] === "year") {
    const payment = await Payment.find({
      year: req.params.id,
    });
...
exports.createPayment = asyncHandler(async (req, res, next) => {
  ...
  if (!customer) {
    return next(
      new ErrorResponse(`No customer with the id of ${req.body.customerId}`),
      404
    );
  }
...
exports.updatePayment = asyncHandler(async (req, res, next) => {
  ...
  payment = await Payment.findByIdAndUpdate({ _id: req.params.id }, req.body, {
    new: true,
    runValidators: true,
  });
